		...
		if (c == 0x01 ){
			...
			switch (sensor) {
				case 0xA0: // Battery Temperature Sensor
					BluetoothSend(mapFloat( (float)batTemp, 0, 1023, 0, 5));
					sensor ++;
					break;
				case 0xA1: // Battery Temperature Sensor
					BluetoothSend(mapFloat( (float)motorTemp, 0, 1023, 0, 5));
					sensor ++;
					break;
				case 0xA2: // Battery Voltage Sensor
					BluetoothSend(mapFloat( (float)batVolt, 0, 1023, 0, 5)); //48
					sensor ++;
					break;
				case 0xA3: // MPPT Current Sensor
					BluetoothSend(mapFloat( (float)mpptCurxTime / (micros() - initialTime1), 0, 1023, 0, 5));
					// Serial.print("\n Current1:  "); // debug
				...
				case 0xA4: // Motor Current Sensor
					BluetoothSend(mapFloat( (float)motorCurxTime / (micros() - initialTime2), 0, 1023, 0, 5));
				...
				case 0xA5: // Potentiometer Voltage
					BluetoothSend(mapFloat( (float)potVolt, 0, 1023, 0, 5));
					sensor = 0xA0;
					break;
				default: 
					sensor = 0xA0;
...
float mapFloat(float value, int in_min, int in_max, int out_min, int out_max){
